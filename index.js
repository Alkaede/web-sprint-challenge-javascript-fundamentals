...
// nestedFunction can only grab the variable internal, because it and the variable internal are anonmyous so the scope in which they reach in is limited to just the function myFunction
...
const zooAnimals = [
  ...
  */
  ...
  function lowerCaseNames(arr){
    let nArr = [];
    arr.map(el => {
      nArr.push(el.animal_name.toLowerCase());
    });
    return nArr;
  }
  ...
  */
  ...
  function lowPopulationAnimals(arr){
    let nArr = arr.filter(el => el.population < 5);
    return nArr;
  }
console.log(lowPopulationAnimals(zooAnimals));
  ...
  */
  ...
  function USApop(arr){
    let totalPop = arr.reduce((total,next) => {
